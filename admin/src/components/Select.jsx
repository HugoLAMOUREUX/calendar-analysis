  ...
  return (
    <Combobox value={value} onChange={onChange}>
      <div className="relative cursor-pointer space-y-3">
        <Combobox.Button
          className={`w-full px-4 py-2 rounded-lg border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all text-left items-center justify-between truncate flex gap-3 ${className}`}
        >
          ...
          <div className="overflow-auto max-h-80">
            {arr.map((string) => (
              <Combobox.Option key={string} value={string}>
                {({ selected, active }) => (
                  <div
                    className={`relative cursor-pointer select-none text-sm py-2 border-0 px-4 transition-colors ${
                      selected ? "bg-indigo-100 text-indigo-900 font-medium" : active ? "bg-gray-100" : ""
                    }`}
                  >
                    {string}
                  </div>
