        ...
        <thead className={`text-left ${sticky ? "sticky top-0 z-10 bg-gray-50 shadow-md" : ""}`}>
          <tr className="border-b border-gray-200">
            {header.map((item, index) => (
              <th
                key={index}
                className={`p-3 py-4 whitespace-nowrap ${item.key && "bg-gray-50 hover:bg-indigo-50 cursor-pointer transition-colors"} ${item.width ? item.width : ""}`}
              ...
              >
                ...
                >
                  ...
                  {item.key && (
                    <button className="flex flex-col justify-center text-indigo-600">
