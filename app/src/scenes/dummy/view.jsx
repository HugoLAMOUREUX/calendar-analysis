...
export default function DummyView() {
  ...
  return (
    <div className="p-6 space-y-6">
      <button onClick={() => navigate("/dummy")} className="text-sm text-gray-600 hover:text-indigo-600 flex items-center gap-2 transition-colors">
    ...
    <div>
      ...
      <div className="flex items-center justify-end gap-2 pt-4 border-t border-gray-200">
        <LoadingButton
          ...
          className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 shadow-lg shadow-indigo-500/30 transition-colors"
        >
          Update
        </LoadingButton>
        <button onClick={handleDelete} className="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700 shadow-lg shadow-red-500/30 transition-colors">
          Delete
        </button>
      </div>
    </div>
...
function RawData({ data }) {
  return (
    <div className="bg-gray-50 p-4 rounded-lg overflow-auto border border-gray-200">
      <pre className="break-all whitespace-pre-wrap text-sm text-gray-800 font-mono">{JSON.stringify(data, null, 2)}</pre>
    </div>
  )
}
