...
import { HiCalendar, HiSparkles, HiArrowPath, HiCheckCircle, HiXCircle } from "react-icons/hi2"
...
export default function List() {
  ...
  const hasUnsyncedCalendars = calendars.some(c => !c.last_event_synced_at)
    ...
    <div className="p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate("/syncing/calendars")}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all disabled:opacity-50 border ${
                hasUnsyncedCalendars ? "bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700 shadow-md" : "bg-white border-gray-200 text-gray-700 hover:bg-gray-50"
              }`}
            >
              <HiArrowPath className={`h-4 w-4 ${hasUnsyncedCalendars ? "animate-spin-slow" : ""}`} />
              <span className="hidden sm:inline">{hasUnsyncedCalendars ? "Sync All Calendars" : "Refresh"}</span>
            </button>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {calendar.last_event_synced_at ? (
                    <div className="flex items-center gap-1 px-1.5 py-0.5 bg-emerald-50 text-emerald-600 rounded-md border border-emerald-100 shadow-sm" title="Synced">
                      <HiCheckCircle className="w-3.5 h-3.5" />
                      <span className="text-[10px] font-bold uppercase tracking-wider">Synced</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-1 px-1.5 py-0.5 bg-red-50 text-red-600 rounded-md border border-red-100 shadow-sm" title="Not synced">
                      <HiXCircle className="w-3.5 h-3.5" />
                      <span className="text-[10px] font-bold uppercase tracking-wider">Not Synced</span>
                    </div>
                  )}
                </div>
