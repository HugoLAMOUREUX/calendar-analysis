...
export default function WrappedSyncing() {
  ...
  const handleSyncCompleted = async () => {
    try {
      const { ok: okUser, user: updatedUser } = await api.get("/user/signin_token")
      if (okUser) setUser(updatedUser)

      const { ok: okSearch, data: wrappeds } = await api.post("/wrapped/search")
      if (!okSearch || !wrappeds.length) throw new Error("Wrapped not found")
  ...
  const getOverallPercentage = () => {
    ...
    // Distribute percentage across steps
    ...
    if (progress.status === "syncing_events" && progress.total > 0) {
      ...
      stepProgress = (1 / STEPS.length) * 50 // Halfway through the step if no specific progress
    }
      ...
      <div className="max-w-xl w-full">
        {status !== "error" && (
          <div className="bg-white rounded-[2.5rem] p-8 lg:p-10 shadow-2xl shadow-indigo-100/50 border border-slate-100 animate-fade-in">
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-black text-slate-900 mb-2 tracking-tight">{status === "completing" ? "C'est presque prêt !" : "Analyse de votre année"}</h1>
              <p className="text-slate-500 text-sm font-medium">Nous préparons votre résumé personnalisé</p>
            </div>
        ...
        {status === "error" && (
          <div className="bg-white rounded-[3rem] p-10 shadow-2xl border border-slate-100 text-center animate-fade-in">
            ...
            <p className="text-slate-500 mb-8 text-sm leading-relaxed">Une erreur est survenue lors de la synchronisation de vos données.</p>
            <button
              onClick={() => window.location.reload()}
              className="w-full bg-slate-900 text-white py-4 rounded-xl font-black hover:bg-slate-800 transition-all shadow-xl shadow-slate-200"
            >
              Réessayer
            </button>
          </div>
